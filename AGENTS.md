...
- The **Crack the Core / War Machine** button calls `api/reference-books-search`: it uses the same **`OPENAI_API_KEY`** as the assistant (when set) to infer **topic + search phrases**, then scores pages in `api/reference_books_index.json`. Without the key it falls back to raw question text. Rebuild the index locally with `python tool/build_reference_book_index.py "<folder-with-pdfs>"` (PyMuPDF); the builder keeps **line breaks** between PDF lines where extraction allows, then commit the JSON for deployment.
