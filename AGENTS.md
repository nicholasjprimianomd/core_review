...
- The **Crack the Core / War Machine** button in the question assistant calls `api/reference-books-search`, which reads `api/reference_books_index.json`. The web app cannot read local PDF paths; build the index on your machine with `python tool/build_reference_book_index.py "<folder-with-pdfs>"` (PyMuPDF required), then commit the updated JSON so production search has content.
